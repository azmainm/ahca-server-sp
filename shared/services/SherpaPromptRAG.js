...
class SherpaPromptRAG {
  ...
  async generateResponse(question, context, conversationHistory = []) {
    try {
      ...
      console.log(`[SherpaPromptRAG][${opId}] 🤖 generateResponse start questionPreview="${promptPreview}${question.length > 160 ? '…' : ''}" contextLength=${context ? context.length : 0}`);
      ...
      console.log(`[SherpaPromptRAG][${opId}] 🧠 ragChain.invoke took ${Date.now() - invokeStart}ms`);
      ...
      if (typeof response === 'string' && response.trim().startsWith('{')) {
        try {
          ...
          if (parsed.answer) {
            console.log(`[SherpaPromptRAG][${opId}] ✅ Parsed structured response in ${Date.now() - totalStart}ms`);
            return parsed;
          }
        } catch (parseError) {
      ...
      console.log(`[SherpaPromptRAG][${opId}] 📝 Returning fallback structured response in ${Date.now() - totalStart}ms`);
      return fallback;
    } catch (error) {
      console.error('Error in SherpaPrompt RAG:', error);
      ...
      console.log(`[SherpaPromptRAG] ❌ Error response returned`);
      return errResp;
    }
  }
...
${section.content}`
  ...
  generateFollowUpQuestions(question, similarContent) {
    ...
    // Analyze content categories to suggest relevant questions for SherpaPrompt
    const categories = [...new Set(similarContent.map(item => item.category))];
    
    if (categories.includes('services') || categories.includes('products')) {
      followUps.push("How does this integrate with my existing tools?");
      followUps.push("Can I schedule a demo to see this in action?");
    }
    
    if (categories.includes('pricing')) {
      followUps.push("Is there a free trial available?");
      followUps.push("What's included in the enterprise plan?");
    }
    
    if (categories.includes('company')) {
      followUps.push("How does SherpaPrompt differ from other automation platforms?");
      followUps.push("What kind of support do you provide?");
    }

    // SherpaPrompt-specific helpful questions
    if (followUps.length < 3) {
      followUps.push("Can you show me how call automation works?");
      followUps.push("What integrations do you support?");
      followUps.push("How quickly can I get started?");
    }
